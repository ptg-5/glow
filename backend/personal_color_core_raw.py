...
try:
    ...
    print("❌ ultralytics 라이브러리가 없습니다. 'pip install ultralytics'를 실행하세요.")
    YOLO = None
...
class PersonalColorSystem:
    def __init__(self):
        print(">>> Backend 로딩: Pure MediaPipe + Auto WB")
        self.pccs = PCCSAnalyzer()
        
        if YOLO:
            try: self.detector = YOLO('yolov8n-face.pt') 
            except: 
                print("기본 모델 사용"); self.detector = YOLO('yolov8n.pt')
    ...
    def analyze_frame(self, frame):
        ...
        # [1] 자동 화이트 밸런스 (float 변환 주의)
        wb_avg = np.mean(frame, axis=(0, 1))
        if np.min(wb_avg) > 10:
            k = np.mean(wb_avg)
            kb, kg, kr = k/wb_avg[0], k/wb_avg[1], k/wb_avg[2]
            frame_wb = frame.astype(np.float32)
            frame_wb[:,:,0] *= kb; frame_wb[:,:,1] *= kg; frame_wb[:,:,2] *= kr
            # [중요] 다시 uint8로 변환해서 저장
            frame = np.clip(frame_wb, 0, 255).astype(np.uint8)
        
        # [2] MediaPipe 처리 (RGB 변환)
        # [수정됨] frame_wb(float)가 아니라 frame(uint8)을 사용해야 함!
        ...
        print(f"[Result] b:{skin_stats['b']:.1f} | Season:{final_season}")
