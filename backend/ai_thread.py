...
<<<<<<< Updated upstream
=======
        self.hef_path_resnet = "/home/intelai/hailo/resnet50_cls.hef"
>>>>>>> Stashed changes
...
<<<<<<< Updated upstream
    ...
    #         # bbox 저장
    #         current_bboxes[label] = [
    #             bbox.xmin(), bbox.ymin(),
    #             bbox.width(), bbox.height()
    #         ]

    #         # 라벨이 "chin:wrinkle:2,pigmentation:1" 형태인지 체크
    #         if ":" in label:
    #             part_name, scores = self.parse_skin_label(label)
    #             if part_name and scores:
    #                 current_frame_data[part_name] = scores
    ...
    #                 # bbox 키 이름 수정
    #                 if label in current_bboxes:
    #                     current_bboxes[part_name] = current_bboxes.pop(label)

    #                 valid_parts_count += 1

    #         elif label in target_parts:
    #             valid_parts_count += 1
    #             # 🔥 중요: 프레임 데이터에 빈 구조라도 넣어줘야
    #             # "회귀값이 없는 초기 프레임" 때문에 측정 실패가 안 난다.
    #             if label not in current_frame_data:
